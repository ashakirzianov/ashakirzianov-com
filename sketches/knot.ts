import {
  fromRGBA, gray, multRGBA, makeStops, toRGBA,
  velocityStep, combineLaws, gravity, circle,
  centerOnMidpoint, zoomToFit, random3d, randomRange, Scene, WithPosition, WithObjects, WithRadius, WithMass, WithVelocity, combineAnimators, Layer, fillGradient,
} from '@/sketcher';

const {
  count, radiusRange, velocityAmp, positionRange,
  palette: { main, complimentary },
} = {
  count: 8,
  velocityAmp: 0.5,
  radiusRange: { min: 0.5, max: 5 },
  positionRange: { min: -100, max: 100 },
  palette: {
    main: toRGBA('orange'),
    complimentary: { red: 230, green: 230, blue: 230 },
  },
};

type KnotObject = WithPosition & WithRadius & WithMass & WithVelocity;
type KnotState = WithObjects<KnotObject>;

export function knot(): Scene<KnotState> {
  let background: Layer<KnotState> = {
    render({ canvas }) {
      fillGradient({
        canvas,
        stops: makeStops({
          0: fromRGBA(complimentary),
          0.2: fromRGBA(multRGBA(complimentary, 1.2)),
          1: gray(255),
        }),
      });
    },
    static: true,
  };

  let foreground: Layer<KnotState> = {
    transforms: [
      zoomToFit({
        widthRange: positionRange,
        heightRange: positionRange,
      }),
      centerOnMidpoint(),
    ],
    render({ state, canvas }) {
      for (let object of state.objects) {
        circle({
          lineWidth: 0.5,
          fill: fromRGBA(main),
          stroke: 'black',
          position: object.position,
          radius: object.radius,
          context: canvas.context,
        });
      }
    },
  };

  let objects = Array(count).fill(undefined).map(() => {
    let radius = randomRange(radiusRange);
    return {
      position: random3d(positionRange),
      velocity: random3d({ min: -velocityAmp, max: velocityAmp }),
      mass: radius,
      radius,
    };
  });

  let animator = combineAnimators<KnotState>({
    objects: (combineLaws(
      gravity({ gravity: 0.2, power: 2 }),
      gravity({ gravity: -0.002, power: 5 }),
      velocityStep(),
    )),
  });

  return {
    state: { objects },
    animator,
    layers: [
      background, foreground,
    ],
  };
}